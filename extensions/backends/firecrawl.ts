 ...
 * grants a free hosted allowance without a key. Bring your own key for
 * higher volume.
 */
...
export async function fetchFirecrawl(
	...
	if (!result) throw new Error(`Firecrawl scrape returned no data for ${url}`);
...
export async function searchFirecrawl(
	...
	const response = await fetch("https://api.firecrawl.dev/v2/search", {
		method: "POST",
		headers,
		body: JSON.stringify({ query, limit: Math.min(numResults, 20) }),
		signal: timeoutSignal(signal),
	});
	if (!response.ok) {
		const text = await response.text().catch(() => "");
